...
**用户故事：** 作为用户，我希望使用管理员为我创建的邮箱和密码登录，以便访问自己的工作空间。

#### 验收标准

1.1 WHEN 用户提交正确的邮箱与密码，THE 系统 SHALL 建立经过服务端验证的登录会话。（实现现状：`packages/auth` 提供 scrypt 密码哈希与应用会话服务，`POST /api/v1/auth/login` 校验密码后下发 `HttpOnly` 会话 Cookie；账号不由用户自助注册，只能由管理员创建。）

1.2 WHEN 用户提交的密码错误、邮箱不存在或账号被禁用，THE 系统 SHALL 拒绝登录并返回完全一致的错误响应，不泄露账户是否存在。（实现现状：未知邮箱与错误密码都执行一次哈希校验，避免通过响应内容或耗时区分账户状态。）

1.3 THE 系统 SHALL 仅保存密码的加盐哈希，不保存也无法反推明文密码；管理员可以重置用户密码，但不能查看原密码。

1.4 WHILE 用户未通过身份验证，THE 系统 SHALL 拒绝访问所有 Agent、Session、附件、产物和用量接口。

1.5 THE 系统 SHALL 仅使用服务端会话中的稳定 `user_id` 标识租户，不接受客户端提交的 `owner_id`。

1.6 WHEN 管理员重置某个用户的密码，THE 系统 SHALL 使该用户已有的全部登录会话失效。
...
7.2 WHILE Agent 正在思考，THE 系统 SHALL 根据 `agent.thinking` 展示非阻塞的思考中状态，且不得伪造思考内容。THE 系统 SHALL 不将推理正文传输到浏览器。

7.3 WHEN Agent 使用内置工具，THE 系统 SHALL 展示工具名称、执行状态和对应结果状态，并 SHALL 展示参数摘要、截断后的结果预览与耗时。THE 系统 SHALL 按调用标识将工具调用与其结果配对；结果状态 SHALL 由方舟的 `is_error` 判定，而非依赖不存在的状态字符串。
...
7.8 WHEN 事件流建立，THE 系统 SHALL 通过 `ready` 事件下发权威运行状态，使中途重连的浏览器无需从回放历史推断即可恢复对应的操作 affordance。该状态 SHALL 取自本地投影，SHALL NOT 为此调用方舟付费接口。

7.9 THE 系统 SHALL 仅以白名单、脱敏并截断的形式向浏览器暴露工具参数与结果，且仅对 Session 所有者可见。THE 系统 SHALL 擦除凭据形状字符串、对象存储键、签名 URL 的查询串与平台 Skill 内部路径，并 SHALL 保留 `/workspace/` 与 `/mnt/session/` 路径。IF 方舟事件类型未列入白名单，THEN THE 系统 SHALL 下发空载荷，SHALL NOT 透传原始 `data`。
...
9.8 THE 系统 SHALL 在产生该产物的 Session 页面内展示其产物列表，并 SHALL 在 Agent 写入文件后刷新该列表。

9.9 THE 系统 SHALL 对浏览器可安全渲染的产物类型（Markdown、纯文本、CSV、图片、HTML）提供内联预览；其余类型 SHALL 仅提供下载。渲染 HTML 时 SHALL 使用不授予同源权限的沙箱 iframe。预览内容 SHALL 仅保存在内存中并在关闭时释放，SHALL NOT 因此产生长期有效的公共 URL。
...
11.4 THE 系统 SHALL 默认使用出站域名白名单，而不是 unrestricted 网络访问。（实现现状：`OUTBOUND_HOST_ALLOWLIST` 仅在 `packages/config` 启动时解析并校验须覆盖配置的 Ark 与 TOS 端点主机，应用运行时不据此实施出站限制；工具执行的实际出站限制由 Ark Environment 的网络配置带外提供。）
...
14.8 THE 系统 SHALL 提供浅色、深色与跟随系统三种外观，SHALL 在本机记住用户选择，并 SHALL 在首帧绘制前应用该选择以避免闪烁。两种配色下正文对比度 SHALL 不低于 WCAG AA。

14.9 THE 系统 SHALL 提供命令面板，使用户无需鼠标即可搜索并跳转到任务、各页面与外观切换。
...
16.6 THE 系统 SHALL 支持通过健康检查确认 Web 服务进程与数据库的可用状态；方舟连接配置 SHALL 在进程启动时校验，就绪探针 SHALL NOT 调用方舟接口或检测方舟连通性。
...
**用户故事：** 作为管理员，我希望在后台创建和维护用户账号，以便控制谁可以登录这个共享工作空间。

#### 验收标准

17.1 THE 系统 SHALL 仅允许管理员访问用户账号管理功能。

17.2 THE 系统 SHALL 允许管理员查看所有账号的邮箱、角色、启用状态以及是否已设置密码。

17.3 THE 系统 SHALL 允许管理员用邮箱、初始密码和角色创建用户账号，且不提供任何自助注册入口。

17.4 IF 管理员使用已存在的邮箱创建账号，THEN THE 系统 SHALL 拒绝该操作并返回邮箱冲突错误。

17.5 THE 系统 SHALL 允许管理员重置任意账号的密码，且重置后使该账号既有会话失效。

17.6 THE 系统 SHALL 不在任何接口或界面中返回密码明文或密码哈希，仅返回"是否已设置密码"。

17.7 WHEN 管理员调整用户的默认 Agent 或配额，THE 系统 SHALL 继续按需求 15 的规则执行。

## 5. 一期完成标准

一期完成时，管理员应能够创建和维护多个平台 Agent，并为每个普通用户指定默认平台 Agent；管理员还应能够在用户账号管理中查看全部账号、用邮箱和初始密码创建账号、重置密码。获得账号的用户应能够用邮箱与密码登录，直接使用默认平台 Agent 创建任务，实时观察执行过程，进行多轮对话或中断任务，上传当前任务附件，并在“我的文件”中查看和下载 Agent 产物。用户还应能够创建和编辑个人 Agent、归档或永久删除任务，并始终无法访问其他用户的任何资源。
